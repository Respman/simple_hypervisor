...
bool InitExecutionControlFields()
{
    ...
    QWORD cr0_fixed0 = rdmsr(IA32_VMX_CR0_FIXED0) & 0x1FFFFFFE;        

    QWORD cr0_fixed1 = rdmsr(IA32_VMX_CR0_FIXED1) & 0xFFFFFFFF;
    ...
    QWORD cr4_mask = (1ull<<13);

    QWORD cr4_fixed0 = rdmsr(IA32_VMX_CR4_FIXED0);        

    QWORD cr4_fixed1 = rdmsr(IA32_VMX_CR4_FIXED1);
            
    cr4_mask = cr4_mask | 
              (~cr4_fixed1) | 
              cr4_fixed0;

    vmwrite(0x6002, cr4_mask);
    ...
    msr_rd_val = rdmsr(IA32_VMX_CR0_FIXED0);
    cr0_fixed0 = msr_rd_val & 0x1FFFFFFE;        

    msr_rd_val = rdmsr(IA32_VMX_CR0_FIXED1);
    cr0_fixed1 = msr_rd_val & 0xFFFFFFFF;
            
    cr0_rd_shadow = cr0_rd_shadow & 
                    cr0_fixed1 | 
                    cr0_fixed0;
    ...
    QWORD cr4_rd_shadow = (1ull<<13);

    msr_rd_val = rdmsr(IA32_VMX_CR4_FIXED0);
    cr4_fixed0 = msr_rd_val & 0x1FFFFFFE;        

    msr_rd_val = rdmsr(IA32_VMX_CR4_FIXED1);
    cr4_fixed1 = msr_rd_val & 0xFFFFFFFF;
            
    cr4_rd_shadow = cr4_rd_shadow & 
                    cr4_fixed1 | 
                    cr4_fixed0;

    vmwrite(0x6006, cr4_rd_shadow);
...
void InitEPT()
{
    ...
    ChangeEPTMemType(HYPERVISOR_base, HyperSpaceUsed, CACHE_TP_UC);
